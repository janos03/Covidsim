...
class Virus():
    def __init__(self, params):
        ...
        self.day_text = self.axes.annotate(
            "Nap: 0", xy=[np.pi / 2, 1], ha="center", va="bottom")
        self.infected_text = self.axes.annotate(
            "Fertőzött: 0", xy=[3 * np.pi / 2, 1], ha="center", va="top", color=RED)
        self.deaths_text = self.axes.annotate(
            "\nHalott: 0", xy=[3 * np.pi / 2, 1], ha="center", va="top", color=BLACK)
        self.recovered_text = self.axes.annotate(
            "\n\nFelépült: 0", xy=[3 * np.pi / 2, 1], ha="center", va="top", color=GREEN)
    ...
    def spread_virus(self, i):
        ...
        if self.day % self.serial_interval == 0 and self.exposed_before < lakos:
            self.num_new_infected = round(self.r0 * self.total_num_infected)
            self.exposed_after += round(self.num_new_infected * 1.1)
            if self.exposed_after > lakos :
                self.num_new_infected = round((lakos - self.exposed_before) * 0.9)
                self.exposed_after = lakos
    ...
    def update_text(self):
        self.day_text.set_text("Nap {}".format(self.day))
        self.infected_text.set_text("Fertőzött: {}".format(self.num_currently_infected))
        self.deaths_text.set_text("\nHalott: {}".format(self.num_deaths))
        self.recovered_text.set_text("\n\nFelépült: {}".format(self.num_recovered))
